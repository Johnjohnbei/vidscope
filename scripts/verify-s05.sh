#!/usr/bin/env bash
# End-to-end verification of M001/S05 — heuristic analyzer.
#
# Usage:
#   bash scripts/verify-s05.sh                    # full run incl integration
#   bash scripts/verify-s05.sh --skip-integration # fast loop, no network
...
if [[ "${SKIP_INTEGRATION}" = true ]]; then
    ...
    printf "\n${CYAN}${BOLD}[%02d] live integration tests (4-stage pipeline)${RESET}\n" "$((step_count + 1))"
    step_count=$((step_count + 1))
    set +e
...
if [[ "${#failed_steps[@]}" -eq 0 ]]; then
    printf "\n${GREEN}${BOLD}✓ S05 verification PASSED${RESET}\n"
    printf "${DIM}Heuristic analyzer is alive. R004 + R010 validated. S06 wires the FTS5 search index.${RESET}\n"
    exit 0
else
    printf "\n${RED}${BOLD}✗ S05 verification FAILED${RESET}\n"
